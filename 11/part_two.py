...
class DumboOctopuseShoal:
    def __init__(self, filename: str):
        ...
        self.flashes = 0
        self.is_everybody_flashed = False
        ...
        for i in range(self.size):
            for j in range(self.size):
                if self.octopuses[i][j] > 9:
                    ...
                    flashed_count += 1
        if flashed_count == self.size * self.size:
            self.is_everybody_flashed = True
        # self.debugprint(self.octopuses)
    ...
    def find_first_synchronisation_step(self) -> None:
        step = 0
        while not self.is_everybody_flashed:
            self.simulate_step()
            step += 1
        return step
...
octopuses = DumboOctopuseShoal("input.txt")
# octopuses.simulate_n_steps(100)
# print(octopuses.get_flashes())
print(octopuses.find_first_synchronisation_step())
