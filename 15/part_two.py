...
class Chiton:
    ...
    def risk_of_shortest_path(self) -> int:
        h0 = len(self.risk_levels)
        h = 5 * h0
        w0 = len(self.risk_levels[0])
        w = 5 * w0
        ...
        while min_risk:
            ...
            for dx, dy in neighbours:
                if not visited[y + dy][x + dx]:
                    extrapolated_risk = self.risk_levels[(y + dy) % h0][(x + dx) % w0]
                    extrapolated_risk += (y + dy) // h0 + (x + dx) // w0
                    extrapolated_risk = extrapolated_risk % 9
                    if extrapolated_risk == 0:
                        extrapolated_risk = 9
                    risk_on_path = least_risked + extrapolated_risk
